    ...
    <el-container>
        ...
        <el-dialog title="新增数据" v-model="addDialogVisible" @close="clean">
            <el-form ref="addData" :model="addForm" label-width="80px">
                ...
                <el-form-item label="版面规格" prop="pageSize">
                    <el-input v-model="addForm.pageSize" placeholder="请输入版面规格"></el-input>
                </el-form-item>
                <el-form-item label="出版社编号" prop="pressId">
                    <el-input v-model="addForm.pressId" placeholder="请输入出版社编号"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="修改数据" v-model="updateDialogVisible" @close="updateFormClean">
            <el-form ref="updateData" :model="updateForm" label-width="80px">
                ...
                <el-form-item label="单价" prop="price">
                    <el-input v-model="updateForm.price" placeholder="请输入单价"></el-input>
                </el-form-item>
                <el-form-item label="版面规格" prop="pageSize">
                    <el-input v-model="updateForm.pageSize" placeholder="请输入版面规格"></el-input>
                </el-form-item>
                <el-form-item label="出版社编号" prop="pressId">
                    <el-input v-model="updateForm.pressId" placeholder="请输入出版社编号"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="() => {
...
const breadcrumbs = ref([
    ...
    { id: "newspaper", title: "报纸管理", url: "/#/sys/newspaper" },
]);
...
const handleAdd = async () => {
    ...
    const res = (await axios.post('/newspaper', addForm.value)).data;
    if (res.code == 200) {
        // 关闭对话框并重置表单
...
const handleDelete = async () => {
    ...
    let ids = selectData.value.map((item) => {
        return item.newspaperId;
    })
    console.log(JSON.stringify(ids));
    const res = (await axios.delete('/newspaper', {
        data: ids
    })).data;
    console.log(res)
    if (res.code == 200) {
...
const handleEdit = async () => {
    console.log(updateForm.value)
    const res = (await axios.put('/newspaper', JSON.stringify(updateForm.value), {
        headers: {
